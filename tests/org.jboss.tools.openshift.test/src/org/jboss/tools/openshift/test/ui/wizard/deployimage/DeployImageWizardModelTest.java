...
public class DeployImageWizardModelTest {
	...
	private TestableDeployImageWizardModel createModel(IProject project, Connection connection,
	...
	public void shouldInitializeContainerInfoFromRemoteDockerImage() throws IOException {
		...
		assertThat(model.getEnvironmentVariables()).contains(
				new EnvironmentVariable("PATH", "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"),
	...
	public void shouldInitializeContainerInfoFromRemoteDockerImageWithVolumes() throws IOException {
		...
		assertThat(model.getPortSpecs()).contains(new PortSpecAdapter("50000-tcp", "TCP", 50000),
	...
	private String getImageStreamImport(final String filename) throws IOException {
		final InputStream imageStreamImport = getClass().getClassLoader().getResourceAsStream(filename);
	...
	public void checkThatRemoveAnExistingServicePortIsEffective() {
		...
		assertThat(model.getServicePorts()).isEqualTo(
				Collections.singletonList(new ServicePortAdapter(new PortSpecAdapter("9990-tcp", "TCP", 9990))));
	}
	...
	public void checkThatServicePortsAreReset() {
		...
		assertThat(model.getServicePorts())
				.isEqualTo(Arrays.asList(first, new ServicePortAdapter(new PortSpecAdapter("9990-tcp", "TCP", 9990))));
	}
	...
	public void checkThatRemoveANonExistingEnvironmentVariableIsNotEffective() {
		...
		assertThat(model.getEnvironmentVariables()).isEqualTo(
				Arrays.asList(new EnvironmentVariable("V1", "value1"), new EnvironmentVariable("V2", "value2")));
	}
	...
	public void shouldInitializeContainerInfoWithEmtyEnvironmentVariable() {
		...
		assertThat(result).isTrue();
		assertThat(model.getEnvironmentVariables()).hasSize(2);
		assertThat(model.getEnvironmentVariables())
				.isEqualTo(Arrays.asList(new EnvironmentVariable("V1", "value1"), new EnvironmentVariable("V2", "")));
	}
