...
public class OpenShiftServerUtilsTest {
    ...
    public void should_throw_exception_no_connection() throws UnsupportedEncodingException, MalformedURLException {
        ...
        // when
        try {
            OpenShiftServerUtils.getDeploymentConfig(OpenShiftServerTestUtils.mockServer(ResourceMocks.PROJECT2_SERVICES[0], connection),
    ...
    @Test
    public void should_throw_exception_no_deployconfig_name_in_pod_labels()
            throws CoreException, UnsupportedEncodingException, MalformedURLException {
        // given
        // when
        try {
            OpenShiftServerUtils.getDeploymentConfig(OpenShiftServerTestUtils.mockServer(ResourceMocks.PROJECT2_SERVICES[3], connection),
                    new NullProgressMonitor());
            // then
            fail("CoreException expected");
        } catch (CoreException e) {
    ...
    public void shouldFindTheFirstServerGivenListOfServersContainsSeveralServerWithSameService() throws CoreException {
        ...
        IServer[] servers = new IServer[] { mockServer("as7", mockServerType(IJBossToolingConstants.SERVER_AS_70)), os3Server1,
                os3Server2 };
        // when
        IServer serverFound = OpenShiftServerUtils.findServerForResource("someService", servers);
        // then
