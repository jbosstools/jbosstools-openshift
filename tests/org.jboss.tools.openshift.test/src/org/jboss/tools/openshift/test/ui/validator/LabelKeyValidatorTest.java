...
public class LabelKeyValidatorTest {
	...
	protected void assertFailure(String value) {
		...
		assertEquals(String.format("Expected to receive ERROR status but got %s", getStatus(act.getSeverity())),
				IStatus.ERROR, act.getSeverity());
	}

	protected void assertCancel(String value) {
		IStatus act = validator.validate(value);
		assertEquals(String.format("CANCEL to receive ERROR status but got %s", getStatus(act.getSeverity())),
