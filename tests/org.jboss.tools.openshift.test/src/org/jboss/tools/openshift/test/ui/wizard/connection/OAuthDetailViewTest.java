...
import org.jboss.tools.openshift.internal.ui.wizard.connection.TokenExtractor;
...
public class OAuthDetailViewTest {
	...
	public void testThatPageWithoutTokenDoesNotMatch() {
		assertFalse(new TokenExtractor("<h2>Your API token is</h2><a href=\"request\">Request another token</a>")
				.isTokenPage());
	}

	@Test
	public void testThatPageWithTokenDoesMatch() {
		TokenExtractor extractor = new TokenExtractor("" + "<style>"
				+ "    body     { font-family: sans-serif; font-size: 14px; margin: 2em 2%; background-color: #F9F9F9; }"
				+ "    h2       { font-size: 1.4em;}" + "    h3       { font-size: 1em; margin: 1.5em 0 0; }"
				...
				+ "    a:hover  { text-decoration: underline; }" + "    @media (min-width: 768px) {"
				+ "        .nowrap { white-space: nowrap; }" + "    }" + "</style>" + "  <h2>Your API token is</h2>"
				+ "  <code>mytoken</code>" +
				...
				+ "<br><br>" + "<a href=\"request\">Request another token</a>");
