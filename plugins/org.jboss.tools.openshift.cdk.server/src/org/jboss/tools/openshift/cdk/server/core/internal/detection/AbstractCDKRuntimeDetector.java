...
public abstract class AbstractCDKRuntimeDetector extends AbstractRuntimeDetectorDelegate {
    ...
    public RuntimeDefinition getRuntimeDefinition(File root, IProgressMonitor monitor) {
        if (validate(root)) {
            RuntimeDefinition def = createDefinition(getDefinitionName(root), getDefinitionVersion(root), getRuntimeDetectionType(), root);
    ...
    protected abstract void initializeServer(IServerWorkingCopy wc, RuntimeDefinition runtimeDefinition) throws CoreException;
