...
public class VagrantPoller extends AbstractCDKPoller {
    ...
    protected boolean checkOpenShiftHealth(String url, int timeout) throws OpenShiftNotReadyPollingException {
        ...
        IClient client = new ClientBuilder(url).sslCertificateCallback(sslCallback).withConnectTimeout(timeout, TimeUnit.MILLISECONDS)
                .build();
        ...
        throw new OpenShiftNotReadyPollingException(CDKCoreActivator.statusFactory().errorStatus(CDKCoreActivator.PLUGIN_ID, msg, e,
    ...
    protected IStatus parseOutput(String[] lines) {
        ...
        if (allStopped(stats)) {
            return CDKCoreActivator.statusFactory()
                    .errorStatus("Vagrant status indicates the CDK is stopped: " + String.join("\n", Arrays.asList(lines)));
        }
