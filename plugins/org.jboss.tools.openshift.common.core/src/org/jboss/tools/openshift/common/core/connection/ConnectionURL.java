...
public class ConnectionURL {
    ...
    public static ConnectionURL safeForConnection(IConnection connection) {
        try {
            ...
            OpenShiftCommonCoreActivator.pluginLog().logError(
                    NLS.bind("Unable to getConnectionURL for connection {0}@{1}", connection.getUsername(), connection.getHost()), e);
    ...
    private static String correctMalformedUrl(String url) {
        ...
        if (StringUtils.isEmpty(matcher.group(4)) && !StringUtils.isEmpty(matcher.group(3))) {
            ...
            return new StringBuilder(matcher.group(3)).append(matcher.group(2)).append('@').append(matcher.group(4)).toString();
        } else {
            return url;
        }
    }
