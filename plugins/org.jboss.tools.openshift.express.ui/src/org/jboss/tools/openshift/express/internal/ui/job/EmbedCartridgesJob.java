...
public class EmbedCartridgesJob extends AbstractDelegatingMonitorJob {
    ...
    protected IStatus doRun(IProgressMonitor monitor) {
        ...
        try {
            removeEmbeddedCartridges(getRemovedCartridges(selectedCartridges, application.getEmbeddedCartridges()), application, monitor);
            this.addedCartridges = addEmbeddedCartridges(getAddedCartridges(selectedCartridges, application.getEmbeddedCartridges()),
                    application, monitor);
            return Status.OK_STATUS;
        } catch (OpenShiftException e) {
            return ExpressUIActivator.createErrorStatus("Could not embed cartridges for application {0}", e, application.getName());
        }
    }
    ...
    private void removeEmbeddedCartridges(List<ICartridge> cartridgesToRemove, final IApplication application, IProgressMonitor monitor)
    ...
    private List<IEmbeddedCartridge> addEmbeddedCartridges(List<ICartridge> cartridgesToAdd, final IApplication application,
        ...
        return application.addEmbeddableCartridges((ICartridge[])cartridgesToAdd.toArray(new ICartridge[cartridgesToAdd.size()]));
    }
