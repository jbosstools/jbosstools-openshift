...
public class PortForwardingWizardPage extends AbstractOpenShiftWizardPage {
    ...
    public PortForwardingWizardPage(final PortForwardingWizardModel wizardModel, final PortForwardingWizard portForwardingWizard) {
        super("Application port forwarding", null, "IApplicationPortForwardingingPage", portForwardingWizard);
    ...
    protected void doCreateControls(Composite parent, DataBindingContext dbc) {
        ...
        GridDataFactory.fillDefaults().span(2, 1).align(SWT.FILL, SWT.CENTER).grab(false, false).applyTo(useLocalIpAddressButton);
        final IObservableValue useLocalIpAddressObservable = BeanProperties
                .value(PortForwardingWizardModel.PROPERTY_USE_DEFAULT_LOCAL_IP_ADDRESS).observe(wizardModel);
        final IObservableValue useLocalIpAddressButtonSelection = WidgetProperties.selection().observe(useLocalIpAddressButton);
        ...
        final IObservableValue findFreePortsButtonObservable = BeanProperties.value(PortForwardingWizardModel.PROPERTY_USE_FREE_PORTS)
                .observe(wizardModel);
        ...
        IObservableValue portForwardingStartedObservable = BeanProperties.value(PortForwardingWizardModel.PROPERTY_PORT_FORWARDING)
                .observe(wizardModel);

        IObservableValue forwardablePortsExistObservable = BeanProperties.value(PortForwardingWizardModel.PROPERTY_FORWARDABLE_PORTS)
                .observe(wizardModel);

        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(startButton)).notUpdating(portForwardingStartedObservable)
                .converting(new InvertingBooleanConverter()).in(dbc);

        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(startButton)).notUpdating(forwardablePortsExistObservable)
                .converting(new Converter(List.class, Boolean.class) {
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(useLocalIpAddressButton)).notUpdating(portForwardingStartedObservable)
                .converting(new InvertingBooleanConverter()).in(dbc);
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(findFreesPortButton)).notUpdating(portForwardingStartedObservable)
                .converting(new InvertingBooleanConverter()).in(dbc);
                        ...
                        protected IStatus run(IProgressMonitor monitor) {
                            ...
                            try {
                                    ...
                                    for (IApplicationPortForwarding portfwd : forwardablePorts) {
                                        if (!portfwd.isStarted(wizardModel.getApplication().getSSHSession())) {
                                            return ExpressUIActivator.createErrorStatus(
                                                    "Failed to start port-forwarding for one or more ports. See console for errors.");
                                        }
                                    }
                                } else {
                                ...
                                return ExpressUIActivator
                                        .createErrorStatus("Problem checking port forward status during start. See console for errors.");
                            }
    ...
    protected TableViewer createTable(Composite tableContainer, DataBindingContext dbc) {
        ...
        IObservableValue forwardablePortsModelObservable = BeanProperties.value(PortForwardingWizardModel.PROPERTY_FORWARDABLE_PORTS)
                .observe(wizardModel);
        ...
        final Job j = new Job("Loading application's forwardable ports...") {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                try {
                    ...
                    return ExpressUIActivator.createErrorStatus("Could not load forwardable ports for application ''{0}''", e,
        ...
        protected IStatus validate() {
            ...
            if (ports == null || ports.isEmpty()) {
                return ValidationStatus
                        .error("There are no available ports to forward for this application.\nYour application may be stopped.");
            }
            return Status.OK_STATUS;
        }
