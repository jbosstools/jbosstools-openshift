...
public class ProjectAndServerAdapterSettingsWizardPage extends AbstractOpenShiftWizardPage {
    ...
    public ProjectAndServerAdapterSettingsWizardPage(IWizard wizard, IOpenShiftApplicationWizardModel wizardModel) {
        super("Set up Project for new OpenShift Application", NLS.bind(PAGE_DESCRIPTION_PATTERN, wizardModel.getApplicationName()),
    ...
    private Composite createProjectControls(Composite parent, DataBindingContext dbc) {
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(projectComposite);
        ...
        newProjectCheckbox.setToolTipText(
        ...
        GridDataFactory.fillDefaults().span(3, 1).align(SWT.FILL, SWT.CENTER).grab(false, false).applyTo(newProjectCheckbox);
        ...
        ValueBindingBuilder.bind(newProjectCheckboxObservable).to(newProjectModelObservable).in(dbc);

        // existing project
        Label existingProjectLabel = new Label(projectComposite, SWT.NONE);
        existingProjectLabel.setText("Use existing project:");
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(false, false).indent(10, 0).applyTo(existingProjectLabel);

        final Text existingProjectNameText = new Text(projectComposite, SWT.BORDER);
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(existingProjectNameText);
        ISWTObservableValue projectNameTextObservable = WidgetProperties.text(SWT.Modify).observe(existingProjectNameText);
        ValueBindingBuilder.bind(projectNameTextObservable)
                .to(BeanProperties.value(ProjectAndServerAdapterSettingsWizardPageModel.PROPERTY_PROJECT_NAME).observe(pageModel)).in(dbc);
        // disable the project name text when the model state is set to 'new project'
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(existingProjectNameText)).notUpdating(newProjectModelObservable)
        ...
        GridDataFactory.fillDefaults().align(SWT.LEFT, SWT.CENTER).hint(100, SWT.DEFAULT).grab(false, false).applyTo(browseProjectsButton);
        browseProjectsButton.addSelectionListener(onBrowseProjects());
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(browseProjectsButton)).notUpdating(newProjectModelObservable)
                .converting(new InvertingBooleanConverter()).in(dbc);
        ...
        final IObservableValue applicationNameModelObservable = BeanProperties
                .value(ProjectAndServerAdapterSettingsWizardPageModel.PROPERTY_APPLICATION_NAME).observe(pageModel);
        final NewProjectValidator newProjectValidator = new NewProjectValidator(newProjectCheckboxObservable,
        ...
        final ExistingProjectValidator existingProjectValidator = new ExistingProjectValidator(newProjectCheckboxObservable,
                projectNameTextObservable);
    ...
    private void createServerAdapterControls(Composite parent, DataBindingContext dbc) {
        ...
        serverAdapterCheckbox.setToolTipText(
                "This Server Adapter will let you publish your local changes onto OpenShift, right from your Eclipse workbench.");
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(serverAdapterCheckbox);
        ValueBindingBuilder.bind(WidgetProperties.selection().observe(serverAdapterCheckbox))
                .to(BeanProperties.value(ProjectAndServerAdapterSettingsWizardPageModel.PROPERTY_CREATE_SERVER_ADAPTER).observe(pageModel))
                .in(dbc);
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(skipMavenBuildCheckbox);
        ValueBindingBuilder.bind(WidgetProperties.selection().observe(skipMavenBuildCheckbox))
                .to(BeanProperties.value(ProjectAndServerAdapterSettingsWizardPageModel.PROPERTY_SKIP_MAVEN_BUILD).observe(pageModel))
                .in(dbc);
    }
        ...
        public IStatus validate() {
            ...
            if (isNewProject) {
                if (StringUtils.isEmptyOrNull(applicationName)) {
                    ...
                    status = ValidationStatus.error("The name may only contain letters and digits.");
                } else {
    ...
    class ExistingProjectValidator extends MultiValidator {
        ...
        private IStatus getGitDirtyStatus(IProject project) {
            IStatus repoCorruptError = ValidationStatus
                    .error(NLS.bind("The git repository for project {0} looks corrupt. Please fix it before using it.", project.getName()));
            try {
                if (EGitUtils.isDirty(project, false, new NullProgressMonitor())) {
                    return ValidationStatus.error(
                            NLS.bind("The project {0} has uncommitted changes. Please commit those changes first.", project.getName()));
                } else {
