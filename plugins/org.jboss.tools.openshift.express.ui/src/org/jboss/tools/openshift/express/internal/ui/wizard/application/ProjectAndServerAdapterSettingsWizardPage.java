...
public class ProjectAndServerAdapterSettingsWizardPage extends AbstractOpenShiftWizardPage {
	...
	public ProjectAndServerAdapterSettingsWizardPage(IWizard wizard, IOpenShiftApplicationWizardModel wizardModel) {
		super("Set up Project for new OpenShift Application",
				NLS.bind(PAGE_DESCRIPTION_PATTERN, wizardModel.getApplicationName()), "Project Configuration", wizard);
	...
	private Composite createProjectControls(Composite parent, DataBindingContext dbc) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(false, false).indent(10, 0)
		...
		ISWTObservableValue projectNameTextObservable = WidgetProperties.text(SWT.Modify)
				.observe(existingProjectNameText);
		ValueBindingBuilder
				.bind(projectNameTextObservable).to(BeanProperties
		...
		final ExistingProjectValidator existingProjectValidator = new ExistingProjectValidator(
				newProjectCheckboxObservable, projectNameTextObservable);
	...
	private void createServerAdapterControls(Composite parent, DataBindingContext dbc) {
		...
		ValueBindingBuilder.bind(WidgetProperties.selection().observe(serverAdapterCheckbox))
				.to(BeanProperties.value(ProjectAndServerAdapterSettingsWizardPageModel.PROPERTY_CREATE_SERVER_ADAPTER)
						.observe(pageModel))
				.in(dbc);
		...
		ValueBindingBuilder.bind(WidgetProperties.selection().observe(skipMavenBuildCheckbox)).to(BeanProperties
				.value(ProjectAndServerAdapterSettingsWizardPageModel.PROPERTY_SKIP_MAVEN_BUILD).observe(pageModel))
				.in(dbc);
	}
	...
	class ExistingProjectValidator extends MultiValidator {
		...
		private IStatus getGitDirtyStatus(IProject project) {
			IStatus repoCorruptError = ValidationStatus
					.error(NLS.bind("The git repository for project {0} looks corrupt. Please fix it before using it.",
							project.getName()));
			try {
				if (EGitUtils.isDirty(project, false, new NullProgressMonitor())) {
					return ValidationStatus.error(
							NLS.bind("The project {0} has uncommitted changes. Please commit those changes first.",
									project.getName()));
				} else {
