...
public class ApplicationTemplateWizardPage extends AbstractOpenShiftWizardPage {
	...
	protected void doCreateControls(Composite container, DataBindingContext dbc) {
		...
		ValueBindingBuilder
				.bind(useExitingApplication).to(BeanProperties
	...
	private AutoCompleteField createExistingAppNameContentAssist(Text existingAppNameText) {
		final AutoCompleteField autoCompleteField = new AutoCompleteField(existingAppNameText, new TextContentAdapter(),
				new String[] {});
	...
	private SelectionListener onBrowseExistingApps() {
		return new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (new OkCancelButtonWizardDialog(getShell(), selectApplicationWizard)
						.open() == IDialogConstants.CANCEL_ID) {
					return;
				}
	...
	protected boolean ensureHasDomain() {
		...
		try {
			...
			WizardDialog dialog = new WizardDialog(Display.getCurrent().getActiveShell(),
					new NewDomainWizard(connection));
			dialog.create();
			...
			new ErrorDialog(getShell(), "Error",
					NLS.bind("Could not lookup domain in OpenShift connection {0}", connection.getId()), status,
					IStatus.ERROR).open();
	...
	protected boolean ensureHasSSHKeys() {
		...
		try {
			...
			WizardDialog dialog = new WizardDialog(Display.getCurrent().getActiveShell(),
					new NoSSHKeysWizard(connection));
			dialog.create();
			dialog.setBlockOnOpen(true);
			return dialog.open() == Dialog.OK;
		} catch (OpenShiftException e) {
			IStatus status = ExpressUIActivator.createErrorStatus(e.getMessage(), e);
			new ErrorDialog(getShell(), "Error",
					NLS.bind("Could not lookup ssh keys in OpenShift connection {0}", connection.getId()), status,
					IStatus.ERROR).open();
			ExpressUIActivator.log(status);
			return false;
		}
		...
		protected IStatus validate() {
			...
			if (application == null) {
				return ValidationStatus
						.error(NLS.bind("You dont have an application called {0} in your domains.", applicationName));
			}
			return ValidationStatus.ok();
	...
	class ApplicationTemplateValidator extends MultiValidator {
		...
		protected IStatus validate() {
			final Boolean useExistingApplication = (Boolean) useExistingApplicationObservable.getValue();
			final IApplicationTemplate selectedApplicationTemplate = (IApplicationTemplate) selectedApplicationTemplateObservable
					.getValue();
			...
			if (selectedApplicationTemplate == null || !selectedApplicationTemplate.canCreateApplication()) {
				return ValidationStatus
						.cancel("Please select an existing cartridge, a quickstart or provide your own framework.");
			}
	...
	private class ApplicationTemplateViewLabelProvider extends StyledCellLabelProvider {
		...
		private void createQuickstartTemplatelabel(StyledString text,
				IQuickstartApplicationTemplate quickstartTemplate) {
