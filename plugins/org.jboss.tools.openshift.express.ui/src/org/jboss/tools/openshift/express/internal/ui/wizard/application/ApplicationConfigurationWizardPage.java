...
public class ApplicationConfigurationWizardPage extends AbstractOpenShiftWizardPage {
    ...
    private void setupWizardModelListeners(OpenShiftApplicationWizardModel wizardModel) {
        wizardModel.addPropertyChangeListener(IOpenShiftApplicationWizardModel.PROP_CONNECTION, new PropertyChangeListener() {
    ...
    private void createDomainControls(Composite parent, DataBindingContext dbc) {
        ...
        domainViewer.setInput(BeanProperties.list(ApplicationConfigurationWizardPageModel.PROPERTY_DOMAINS).observe(pageModel));
        ...
        ValueBindingBuilder.bind(selectedDomainObservable)
            ...
            public void handleValueChange(ValueChangeEvent event) {
                ...
                try {
                    WizardUtils.runInWizard(
                            new AbstractDelegatingMonitorJob(NLS.bind("Loading applications for domain {0}...", domain.getId())) {
                    ...
                    ExpressUIActivator
                            .log(ExpressUIActivator.createErrorStatus(NLS.bind("Could not load applications for domain {0}.", domain), e));
    ...
    private void createApplicationControls(Composite parent, DataBindingContext dbc) {
        ...
        GridDataFactory.fillDefaults().grab(true, false).span(2, 1).align(SWT.FILL, SWT.FILL).applyTo(applicationNameText);
        UIUtils.selectAllOnFocus(applicationNameText);
        final IObservableValue applicationNameTextObservable = WidgetProperties.text(SWT.Modify).observe(applicationNameText);
        ...
        ControlDecorationSupport.create(newApplicationNameValidator, SWT.LEFT | SWT.TOP, null, new RequiredControlDecorationUpdater());
        ...
        GridDataFactory.fillDefaults().span(2, 1).align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(selectedApplicationTemplateLabel);
        IObservableValue selectedApplicationTemplate = BeanProperties
                .value(ApplicationConfigurationWizardPageModel.PROPERTY_SELECTED_APPLICATION_TEMPLATE).observe(pageModel);
        ValueBindingBuilder.bind(BeanProperties.value(IApplicationTemplate.PROPERTY_NAME).observeDetail(selectedApplicationTemplate))
                ...
                }).to(WidgetProperties.text().observe(selectedApplicationTemplateLabel)).notUpdatingParticipant().in(dbc);
        ...
        gearViewer.setInput(BeanProperties.list(ApplicationConfigurationWizardPageModel.PROPERTY_GEAR_PROFILES).observe(pageModel));
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(gearViewer.getControl());
        ValueBindingBuilder.bind(ViewerProperties.singlePostSelection().observe(gearViewer))
                .to(BeanProperties.value(ApplicationConfigurationWizardPageModel.PROPERTY_SELECTED_GEAR_PROFILE).observe(pageModel))
                .in(dbc);
        ...
        ValueBindingBuilder.bind(WidgetProperties.selection().observe(enableScalingButton))
                ...
                .to(BeanProperties.value(ApplicationConfigurationWizardPageModel.PROPERTY_APPLICATION_SCALE).observe(pageModel))
        ...
        GridDataFactory.fillDefaults().grab(true, true).align(SWT.FILL, SWT.FILL).span(3, 1).applyTo(embeddableCartridgesGroup);
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).hint(400, SWT.DEFAULT).grab(true, true).span(1, 2).applyTo(tableContainer);
        TableViewer embeddableCartridgesTableViewer = createEmbeddableCartridgesViewer(tableContainer);
        embeddableCartridgesTableViewer
                .setInput(BeanProperties.set(ApplicationConfigurationWizardPageModel.PROPERTY_EMBEDDED_CARTRIDGES).observe(pageModel));
        ValueBindingBuilder.bind(ViewerProperties.singlePostSelection().observe(embeddableCartridgesTableViewer))
                .to(BeanProperties.value(ApplicationConfigurationWizardPageModel.PROPERTY_SELECTED_CARTRIDGE).observe(pageModel)).in(dbc);
    ...
    private Composite createAddRemoveEditButtons(Group parent, DataBindingContext dbc) {
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(removeButton)).notUpdatingParticipant().to(selectedEmbeddableCartridge)
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(editButton)).notUpdatingParticipant().to(selectedEmbeddableCartridge)
                .converting(new IsNotNull2BooleanConverter()).in(dbc);

        stackLayout.topControl = getEmbeddableCartridgesButtons(pageModel.isCanAddRemoveCartridges(), addRemoveButtons, editButtons);
    ...
    private IValueChangeListener onCanAddRemoveChanged(final Composite addRemoveButtons, final Composite editButtons,
        ...
        return new IValueChangeListener() {
            ...
            public void handleValueChange(ValueChangeEvent event) {
                ...
                stackLayout.topControl = getEmbeddableCartridgesButtons((Boolean)event.diff.getNewValue(), addRemoveButtons, editButtons);
                parent.layout(true);
    ...
    private Composite getEmbeddableCartridgesButtons(boolean canAddRemoveEmbeddableCartridges, Composite addRemoveButtons,
            Composite editButtons) {
    ...
    private void doCreateAdvancedContent(Composite advancedComposite, DataBindingContext dbc) {
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(sourceUrlLabel)).notUpdatingParticipant().to(sourceUrlWidgetsEnablement)
                .in(dbc);
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(sourceUrlText)).notUpdatingParticipant().to(sourceUrlWidgetsEnablement)
                .in(dbc);
        ...
        ValueBindingBuilder.bind(sourcecodeUrlObservable)
                ...
                .to(BeanProperties.value(ApplicationConfigurationWizardPageModel.PROPERTY_INITIAL_GITURL).observe(pageModel)).in(dbc);

        MultiValidator sourceCodeUrlValidator = new SourceCodeUrlValidator(useDefaultSourceButtonObservable, sourcecodeUrlObservable);
        dbc.addValidationStatusProvider(sourceCodeUrlValidator);
        ControlDecorationSupport.create(sourceCodeUrlValidator, SWT.LEFT | SWT.TOP, null, new RequiredControlDecorationUpdater());
        ...
        sourceCodeExplanationText.setText("Your application will start with an exact copy of the code and configuration "
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, true).span(2, 1).applyTo(sourceCodeExplanationText);
        ...
        ValueBindingBuilder.bind(WidgetProperties.visible().observe(environmentVariablesButton))
                .notUpdatingParticipant().to(BeanProperties
                        .value(ApplicationConfigurationWizardPageModel.PROPERTY_ENVIRONMENT_VARIABLES_SUPPORTED).observe(pageModel))
                .in(dbc);
    }
    ...
    private TableViewer createEmbeddableCartridgesViewer(Composite tableContainer) {
        ...
        TableViewer viewer = new TableViewerBuilder(table, tableContainer).sorter(new EmbeddableCartridgeViewerSorter())
                .comparer(new EqualityComparer()).contentProvider(new ObservableSetContentProvider()).<ICartridge>column("Name").weight(1)
    ...
    private SelectionListener onRemove() {
        return new SelectionAdapter() {
            ...
            private void remove(ICartridge cartridge) {
                ...
                EmbeddableCartridgeDiff additionalOperations = embedCartridgeStrategy.remove(cartridge, pageModel.getEmbeddedCartridges());
            ...
            protected int executeAdditionalOperations(ICartridge cartridge, EmbeddableCartridgeDiff additionalOperations) {
    ...
    private SelectionListener onEdit() {
        return new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                EditAlternativeCartridgesWizard cartridgesWizard = new EditAlternativeCartridgesWizard(selectedCartridge,
                        selectedQuickstart.getAlternativesFor(selectedCartridge), wizardModel);
                if (new OkCancelButtonWizardDialog(getShell(), cartridgesWizard).open() == IDialogConstants.OK_ID) {
    ...
    class NewApplicationNameValidator extends MultiValidator {
        ...
        NewApplicationNameValidator(IObservableValue useExistingApplication, IObservableValue existingApplicationsLoaded,
                IObservableValue applicationName) {
