...
public class ManageDomainsWizardPage extends AbstractOpenShiftWizardPage {
	...
	private SelectionListener onRemove(final DataBindingContext dbc) {
		return new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				Job deleteDomainJob = new JobChainBuilder(
						new DestroyDomainJob(domain, dialog.isForceDelete()))
						.andRunWhenSuccessfull(
								new RefreshConnectionsModelJob(pageModel.getConnection()))
						.build();
				try {
					org.jboss.tools.common.ui.WizardUtils.runInWizard(
							deleteDomainJob, new DelegatingProgressMonitor(), getContainer(), dbc);
	...
	private void refreshModel(final DataBindingContext dbc) {
		try {
			org.jboss.tools.common.ui.WizardUtils.runInWizard(
					new AbstractDelegatingMonitorJob("Refreshing connections") {
			...
			Logger.error(NLS.bind("Could not refresh domain {0}", pageModel.getConnection().getUsername()), ex);
		} catch (InterruptedException ex) {
			Logger.error(NLS.bind("Could not refresh domain {0}", pageModel.getConnection().getUsername()), ex);
		}
	}
