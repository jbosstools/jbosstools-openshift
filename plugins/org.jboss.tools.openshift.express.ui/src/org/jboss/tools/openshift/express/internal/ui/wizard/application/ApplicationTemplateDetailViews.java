...
public class ApplicationTemplateDetailViews extends AbstractStackedDetailViews {
    ...
    private class CodeAnthingCartridgeView extends DefaultView {
        ...
        public void onVisible(IObservableValue applicationTemplateObservable, DataBindingContext dbc) {
            ...
            this.binding = ValueBindingBuilder.bind(urlTextObservable)
                    ...
                    .in(dbc);
            CodeAnythingCartridgeUrlValidator codeAnythingCartridgeUrlValidator = new CodeAnythingCartridgeUrlValidator(urlTextObservable,
                    applicationTemplateObservable, disabled);
    ...
    private class QuickstartView extends EmptyView {
        ...
        public void onVisible(IObservableValue applicationTemplateObservable, DataBindingContext dbc) {
            ...
            this.nameLink.setText(new StringBuilder().append("<a>").append(template.getName()).append("</a>").toString());
        ...
        private void updateOpenShiftMaintainedIcon(IQuickstartApplicationTemplate template) {
            if (template.isOpenShiftMaintained()) {
                setImageAndTooltip(openshiftMaintainedLabel, "OpenShift maintained", ExpressImages.OPENSHIFT_MAINTAINED_IMG);
            } else {
                setImageAndTooltip(openshiftMaintainedLabel, "Community created", ExpressImages.NOT_OPENSHIFT_MAINTAINED_IMG);
            }
        }

        private void updateSecurityUpdatesIcon(IQuickstartApplicationTemplate template) {
            if (template.isAutomaticSecurityUpdates()) {
                setImageAndTooltip(securityUpdatesLabel, "automatic security updates", ExpressImages.SECURITY_UPDATES_IMG);
            } else {
                setImageAndTooltip(securityUpdatesLabel, "no automatic security updates", ExpressImages.NO_SECURITY_UPDATES_IMG);
            }
        }
        ...
        private SelectionListener onLinkClicked() {
            return new SelectionAdapter() {
                ...
                public void widgetSelected(SelectionEvent e) {
                    ...
                    new BrowserUtility().checkedCreateExternalBrowser(template.getPageUrl(), ExpressUIActivator.PLUGIN_ID,
