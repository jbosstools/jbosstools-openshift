...
public class ApplicationTemplateDetailViews extends AbstractStackedDetailViews {
	...
	ApplicationTemplateDetailViews(IObservableValue detailViewModel, IObservableValue disabled, Composite parent,
	...
	private class DefaultView extends EmptyView {
		...
		public void onVisible(IObservableValue applicationTemplateObservable, DataBindingContext dbc) {
			...
			if (!(value instanceof IApplicationTemplate) || DisposeUtils.isDisposed(nameText)) {
				return;
			}
	...
	private class CodeAnthingCartridgeView extends DefaultView {
		...
		public Composite createControls(Composite parent, Object context, DataBindingContext dbc) {
			...
			this.nameText = new StyledText(container, SWT.READ_ONLY);
			StyledTextUtils.setTransparent(nameText);
			GridDataFactory.fillDefaults().span(2, 1).align(SWT.LEFT, SWT.CENTER).grab(true, false).applyTo(nameText);
		...
		private void createContentProposal(Text text) {
			final ControlDecoration decoration = ContentProposalUtils
					.createContenProposalDecoration("History available", text);
			ContentProposalUtils.createContentProposal(text,
					ExpressCorePreferences.INSTANCE.getDownloadableStandaloneCartUrls());
		...
		public void onVisible(IObservableValue applicationTemplateObservable, DataBindingContext dbc) {
			...
			this.binding = ValueBindingBuilder.bind(urlTextObservable)
					...
					.in(dbc);
			CodeAnythingCartridgeUrlValidator codeAnythingCartridgeUrlValidator = new CodeAnythingCartridgeUrlValidator(
					urlTextObservable, applicationTemplateObservable, disabled);
			dbc.addValidationStatusProvider(codeAnythingCartridgeUrlValidator);
			ControlDecorationSupport.create(codeAnythingCartridgeUrlValidator, SWT.LEFT | SWT.TOP, null,
		...
		class CodeAnythingCartridgeUrlValidator extends MultiValidator {
			...
			private CodeAnythingCartridgeUrlValidator(IObservableValue url, IObservableValue template,
					IObservableValue disabled) {
				this.url = url;
	...
	private class QuickstartView extends EmptyView {
		...
		public Composite createControls(Composite parent, Object context, DataBindingContext dbc) {
			...
			GridDataFactory.fillDefaults().align(SWT.LEFT, SWT.CENTER).applyTo(nameLink);
			nameLink.addSelectionListener(onLinkClicked());

			// icons
			this.openshiftMaintainedLabel = new CLabel(container, SWT.None);
			GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).applyTo(openshiftMaintainedLabel);
			this.securityUpdatesLabel = new CLabel(container, SWT.None);
			GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).applyTo(securityUpdatesLabel);
			...
			GridDataFactory.fillDefaults().span(3, 1).align(SWT.FILL, SWT.FILL).grab(true, true)
					.hint(container.getClientArea().x, SWT.DEFAULT).applyTo(summaryText);
			return container;
		}
		...
		public void onVisible(IObservableValue applicationTemplateObservable, DataBindingContext dbc) {
			Object value = applicationTemplateObservable.getValue();
			if (!(value instanceof IQuickstartApplicationTemplate) || DisposeUtils.isDisposed(nameLink)) {
				return;
			}
			this.template = (IQuickstartApplicationTemplate) value;
			this.nameLink
					.setText(new StringBuilder().append("<a>").append(template.getName()).append("</a>").toString());
		...
		private void updateOpenShiftMaintainedIcon(IQuickstartApplicationTemplate template) {
			if (template.isOpenShiftMaintained()) {
				setImageAndTooltip(openshiftMaintainedLabel, "OpenShift maintained",
						ExpressImages.OPENSHIFT_MAINTAINED_IMG);
			} else {
				setImageAndTooltip(openshiftMaintainedLabel, "Community created",
		...
		private void updateSecurityUpdatesIcon(IQuickstartApplicationTemplate template) {
			if (template.isAutomaticSecurityUpdates()) {
				setImageAndTooltip(securityUpdatesLabel, "automatic security updates",
						ExpressImages.SECURITY_UPDATES_IMG);
			} else {
				setImageAndTooltip(securityUpdatesLabel, "no automatic security updates",
		...
		private SelectionListener onLinkClicked() {
			return new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					new BrowserUtility().checkedCreateExternalBrowser(template.getPageUrl(),
