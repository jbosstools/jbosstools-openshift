    ...
    public boolean performFinish() {
        ...
        try {
            ...
            Job jobChain = new JobChainBuilder(restoreJob)
                    .runWhenDone(new UIJob(NLS.bind("Show Snapshot Restore/Deploy Output for application {0}...", applicationName)) {
                        ...
                        public IStatus runInUIThread(IProgressMonitor monitor) {
                            ...
                            if (console == null) {
                                return ExpressUIActivator
                                        .createCancelStatus(NLS.bind("Cound not open console for application {0}", applicationName));
                            }
            ...
            new ErrorDialog(getShell(), "Error", NLS.bind("Could not restore snapshot for application {0}", applicationName), status,
                    IStatus.ERROR).open();
            return false;
    ...
    private class RestoreJob extends AbstractDelegatingMonitorJob {
        ...
        protected IStatus doRun(IProgressMonitor monitor) {
            ...
            try {
                ...
                return ExpressUIActivator
                        .createErrorStatus(NLS.bind("Could not restore snapshot for application {0}", application.getName()), e);
            } finally {
                if (getModel().isDeploymentSnapshot()) {
                    // restore
