            ...
            try {
                ...
                elements.add(new StringElement("SSH ExpressConnection", new ApplicationPropertyGetter(application) {
                    ...
                    protected String doGet(IApplication application) {
                        ...
                        return new StringBuilder(EGitUtils.getGitUsername(gitUrl)).append('@').append(EGitUtils.getGitHost(gitUrl))
                                .toString();
                    }
                }.safeGet()));
                ...
                Logger.error(NLS.bind("Could not display details for OpenShift application {0}", application.getName()), e);
            }
        }
        return elements.toArray();
    }

    private ContainerElement createCartridges(ContainerElement cartridgesContainer, IApplication application) throws OpenShiftException {
