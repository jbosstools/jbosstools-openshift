...
public class SaveSnapshotWizardPage extends AbstractOpenShiftWizardPage {
    ...
    public SaveSnapshotWizardPage(SaveSnapshotWizardModel wizardModel, IWizard wizard) {
        this("Save Snapshot Cartridges", NLS.bind("Please choose the snapshot type and file for the snapshot of application {0}",
                StringUtils.null2emptyString(wizardModel.getApplication().getName())), wizardModel, wizard);
    }
    ...
    protected void doCreateControls(Composite parent, DataBindingContext dbc) {
        ...
        ValueBindingBuilder.bind(WidgetProperties.selection().observe(deploymentSnapshotButton))
                .to(BeanProperties.value(SaveSnapshotWizardPageModel.PROPERTY_DEPLOYMENT_SNAPSHOT).observe(pageModel)).in(dbc);
