...
public class EditDomainWizardModel extends ObservableUIPojo {
	...
	public String getWebUIDomainPageUrl() {
		...
		return createWebUIDomainPageUrl(getDomainId(), getDomain().getUser().getServer());
	}

	private String createWebUIDomainPageUrl(String domainId, String host) {
		String domainWebUIUrl = MessageFormat.format(OPENSHIFT_ORIGIN_WEBUI_DOMAINPAGE, host, domainId);
		if (isUrlAvailable(domainWebUIUrl)) {
			return domainWebUIUrl;
		}
		domainWebUIUrl = MessageFormat.format(OPENSHIFT_ENTERPRISE_WEBUI_DOMAINPAGE, host, domainId);
		if (isUrlAvailable(domainWebUIUrl)) {
			return domainWebUIUrl;
		}
		return null;
