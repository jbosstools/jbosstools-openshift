...
public class ExpressConnection extends AbstractConnection {
    ...
    protected ExpressConnection(String username, String password, String scheme, String host, boolean rememberPassword, IUser user,
    ...
    protected IUser doCreateUser() {
        ...
        try {
            return new ConnectionBuilder(getHost()).credentials(username, password).clientId(userId).sslCertificateCallback(sslCallback)
                    .create().getUser();
        } catch (IOException e) {
    ...
    public IApplication createApplication(final String applicationName, final IStandaloneCartridge standaloneCartridge,
            final ApplicationScale scale, final IGearProfile gearProfile, final IDomain domain) throws OpenShiftException {
        if (connect()) {
