...
public class ExpressServerUtils {
    ...
    public static String getApplicationName(IServerAttributes attributes) {
        return getProjectAttribute(SETTING_APPLICATION_NAME, attributes.getAttribute(ATTRIBUTE_APPLICATION_NAME, (String)null),
                getDeployProject(attributes));
    }

    public static String getApplicationId(IServerAttributes attributes) {
        return getProjectAttribute(SETTING_APPLICATION_ID, attributes.getAttribute(ATTRIBUTE_APPLICATION_ID, (String)null),
                getDeployProject(attributes));
    }
    ...
    public static String getRemoteName(IServerAttributes attributes) {
        if (isOverridesProject(attributes)) {
            ...
            return getProjectAttribute(SETTING_REMOTE_NAME, ATTRIBUTE_REMOTE_NAME_DEFAULT, getDeployProject(attributes));
        }
    }
    ...
    public static IServer fillServerWithOpenShiftDetails(IServer server, String deployProject, String remote, String serverName,
            IApplication application, IDomain domain) throws CoreException {
    ...
    public static void fillServerWithOpenShiftDetails(IServerWorkingCopy wc, String serverName, IProject deployProject, String deployFolder,
            String remote, IApplication application, IDomain domain) {
        String host = getHost(application);
     ...
     */
    public static void fillServerWithOpenShiftDetails(IServerWorkingCopy wc, String serverName, String host, String deployProject,
            String deployFolder, String remote, String applicationName, String domainName) {
    ...
    public static void updateOpenshiftProjectSettings(IProject project, IApplication app, IDomain domain, ExpressConnection connection,
    ...
    private static void setConnectionUrl(IConnection connection, IEclipsePreferences node) {
        try {
            ...
            ExpressCoreActivator.pluginLog().logError(
                    NLS.bind("Could not get connection url for connection {0}/{1}", connection.getUsername(), connection.getHost()), e);
        } catch (MalformedURLException e) {
            ExpressCoreActivator.pluginLog().logError(
                    NLS.bind("Could not get connection url for connection {0}/{1}", connection.getUsername(), connection.getHost()), e);
        }
