    ...
    protected int pollState(IProgressMonitor monitor) {
        ...
        if (resource == null) {
            OpenShiftCoreActivator.pluginLog()
                    .logError("The OpenShift resource for server " + getServer().getName() + " could not be reached.", e);
    ...
    protected int mapPortForwarding(final DebugContext context, final IProgressMonitor monitor) throws CoreException {
        ...
        if (!debugPort.isPresent()) {
            throw new CoreException(StatusFactory.errorStatus(OpenShiftCoreActivator.PLUGIN_ID, NLS.bind(
                    "Pod port specified in server adapter \"{0}\" is no present in pod \"{1}\"", getServer().getName(), pod.getName())));
        }
        ...
        throw new CoreException(StatusFactory.errorStatus(OpenShiftCoreActivator.PLUGIN_ID, NLS
                .bind("Could not setup port forwarding to pod \"{0}\" in server adapter \"{1}\"", pod.getName(), getServer().getName())));
    }
