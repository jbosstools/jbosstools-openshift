...
public class KeyValueWizardModelBuilder<T extends IKeyValueItem> {
    ...
    public IKeyValueWizardModel<T> build() {
        KeyValueWizardModel<T> model = new KeyValueWizardModel<>(windowTitle, title, keyLabel, valueLabel, description, groupLabel,
                keyAfterConvertValidator, valueAfterConvertValidator, keyEditable);
        model.setKey(key);
