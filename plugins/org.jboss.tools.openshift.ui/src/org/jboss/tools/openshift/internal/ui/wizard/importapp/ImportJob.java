    ...
    public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
        try {
            if (reuseGitRepository) {
                this.importedProjects = new ImportProjectOperation(gitUrl, gitRef, cloneDestination, filters, checkoutBranch)
                        .execute(monitor);
            } else {
            ...
            return OpenShiftUIActivator.statusFactory().errorStatus(NLS.bind("Could not import project from {0}.", e, gitUrl));
        } catch (IOException e) {
            return OpenShiftUIActivator.statusFactory().errorStatus(NLS.bind("Could not import project from {0}.", e, gitUrl));
            ...
            if (te != null) {
                return OpenShiftUIActivator.statusFactory().errorStatus("Could not clone the repository. Authentication failed.\n"
                        + " Please make sure that you added your private key to the ssh preferences.", te);
            } else {
                return OpenShiftUIActivator.statusFactory().errorStatus("An exception occurred while creating local git repository.", e);
            }
        } catch (CoreException e) {
            return StatusFactory.getMultiStatusInstance(0, OpenShiftUIActivator.PLUGIN_ID, "Could not import project to the workspace.",
                    null, e.getStatus());
