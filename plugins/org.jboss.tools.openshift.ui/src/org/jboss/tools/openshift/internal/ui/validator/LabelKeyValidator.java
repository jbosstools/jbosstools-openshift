...
public class LabelKeyValidator extends LabelValueValidator {
    ...
    public IStatus validate(Object paramObject) {
        ...
        case 2:
            if (!validateSubdomain(parts[0])) {
                if (parts[0].length() > SUBDOMAIN_MAXLENGTH) {
                    return ValidationStatus
                            .error(NLS.bind("Maximum length of domain of label key allowed is {0} characters", SUBDOMAIN_MAXLENGTH));
                }
            ...
            if (!validateLabel(parts[1])) {
                if (value.length() > LABEL_MAXLENGTH) {
                    return ValidationStatus
                            .error(NLS.bind("Maximum length of name of label key allowed is {0} characters", LABEL_MAXLENGTH));
                }
        ...
        return ValidationStatus
                .error(NLS.bind("More than one '/' is not allowed. A valid {0} has an optional domain separated by '/' from name", type));
    }
