...
public class OAuthDetailView extends BaseDetailsView implements IConnectionEditorDetailView {
    ...
    public static class TokenExtractor {
         ...
         */
        public static final Pattern TOKEN_PAGE_PATTERN = Pattern.compile(".*<h2>Your API token is<\\/h2>.*<code>(.*)<\\/code>.*",
                Pattern.DOTALL);
    ...
    public OAuthDetailView(IWizard wizard, ConnectionWizardPageModel pageModel, IValueChangeListener<?> changeListener, Object context,
        ...
        this.connectionValidator = ConnectionValidatorFactory.createOAuthAuthenticationValidator(pageModel, tokenObservable, urlObservable);
    ...
    public Composite createControls(Composite parent, Object context, DataBindingContext dbc) {
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).span(2, 1).grab(true, false).applyTo(rememberTokenCheckbox);
    ...
    public void onVisible(IObservableValue detailsViewModel, DataBindingContext dbc) {
        ...
        this.rememberTokenBinding = ValueBindingBuilder.bind(WidgetProperties.selection().observe(rememberTokenCheckbox))
                .to(rememberTokenObservable).in(dbc);
    }
                        ...
                        public void run() {
                            ...
                            if (details != null) {
                                ...
                                if (IAuthorizationContext.AUTHSCHEME_BASIC.equals(details.getScheme())) {
                                    MessageDialog.openError(shell, "Authorization Information",
                                            NLS.bind("This server utilizes {0} authorization protocol", details.getScheme()));
    ...
    private void showErrorDialog(final Shell shell, final Throwable e) {
        Display.getDefault().asyncExec(new Runnable() {
            ...
            @Override
            public void run() {
                ErrorDialog.openError(shell, "Request Authentication Details Error", "Unable to retrieve authentication details",
        ...
        protected IStatus run(IProgressMonitor monitor) {
            try {
                IClient client = new ClientBuilder(host)
                        .sslCertificateCallback(OpenShiftCoreUIIntegration.getInstance().getSSLCertificateCallback()).build();
