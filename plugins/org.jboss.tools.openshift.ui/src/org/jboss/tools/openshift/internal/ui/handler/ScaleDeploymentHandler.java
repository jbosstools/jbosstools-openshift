...
public class ScaleDeploymentHandler extends AbstractHandler {
    ...
    private IDeploymentConfig getDeploymentConfig(IServiceWrapper service) {
        return ResourceUtils.getLatestResourceVersion(service.getResourcesOfKind(ResourceKind.DEPLOYMENT_CONFIG).stream()
                .map(wrapper -> (IDeploymentConfig)wrapper.getWrapped()).collect(Collectors.<IDeploymentConfig>toList()));
    }
    ...
    protected boolean showStopDeploymentWarning(String name, Shell shell) {
        MessageDialog dialog = new MessageDialog(shell, "Stop all deployments?", OpenShiftCommonImages.OPENSHIFT_LOGO_WHITE_ICON_IMG,
    ...
    public class ScaleReplicasDialog extends TitleAreaDialog {
        ...
        protected Control createDialogArea(Composite parent) {
            ...
            ValueBindingBuilder.bind(WidgetProperties.selection().observe(scaleSpinner)).validatingAfterConvert(new IValidator() {

                @Override
                public IStatus validate(Object value) {
                    if (!(value instanceof Integer)) {
                        return ValidationStatus
                                .error(NLS.bind("You need to provide a positive number of replicas for deployment {0}", name));
                    }
