...
public class PortForwardingWizardModel extends ObservablePojo {
    ...
    public void startPortForwarding() {
        ...
        try (MessageConsoleStream stream = console.newMessageStream()) {
            ...
            portForwardable.getPortPairs().stream().forEach(port -> stream
                    .println(NLS.bind("{0} {1} -> {2}", new Object[] { port.getName(), port.getLocalPort(), port.getRemotePort() })));
            stream.println("done.");
    ...
    public void stopPortForwarding() throws IOException {
        ...
        try (final MessageConsoleStream stream = console.newMessageStream()) {
            ...
            if (cap != null) {
                cap.getPortPairs().stream().forEach(port -> stream
                        .println(NLS.bind("{0} {1} -> {2}", new Object[] { port.getName(), port.getLocalPort(), port.getRemotePort() })));
            }
