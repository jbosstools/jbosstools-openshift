 ...
 */
public class LinkComponentHandler extends OdoHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			if (targetComponents.isEmpty()) {
				MessageDialog.openError(HandlerUtil.getActiveShell(event), "Link component", "No component available to link to.");
			} else {
				...
				if (WizardUtils.openWizardDialog(linkComponentWizard, HandlerUtil.getActiveShell(event)) == Window.OK) {
					executeInJob("Link component", () -> execute(HandlerUtil.getActiveShell(event), model, component));
				}
			}
