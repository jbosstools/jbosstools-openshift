...
public class DeployImagePage extends AbstractOpenShiftWizardPage {
    ...
    protected void onPageWillGetDeactivated(final Direction progress, final PageChangingEvent event, final DataBindingContext dbc) {
        ...
        try {
            ...
            if (!validatorJobStatus.isOK()) {
                MessageDialog.openError(getShell(), "Error", NLS.bind("No Docker image named {0} could be found.", model.getImageName()));
                event.doit = false;
            }
    ...
    private void createDockerConnectionControl(Composite parent, DataBindingContext dbc) {
        ...
        Binding selectedConnectionBinding = ValueBindingBuilder.bind(selectedConnectionObservable)
                .converting(new ObservableTreeItem2ModelConverter(IDockerConnection.class)).validatingAfterConvert(validator)
    ...
    private void createOpenShiftConnectionControl(Composite parent, DataBindingContext dbc) {
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).span(NUM_COLUMS - 1, 1).grab(true, false).applyTo(connectionText);
        final IObservableValue<String> connnectionTextObservable = WidgetProperties.text(SWT.None).observe(connectionText);
        final IObservableValue<IConnection> connnectionObservable = BeanProperties.value(IDeployImagePageModel.PROPERTY_CONNECTION)
                .observe(model);
        ValueBindingBuilder.bind(connnectionTextObservable).notUpdatingParticipant().to(connnectionObservable)
    ...
    private void createProjectControl(Composite parent, DataBindingContext dbc) {
        ...
        Binding selectedProjectBinding = ValueBindingBuilder.bind(selectedProjectObservable)
                .converting(new ObservableTreeItem2ModelConverter(IProject.class)).validatingAfterConvert(validator).to(projectObservable)
                .in(dbc);
    ...
    private void createImageNameControls(final Composite parent, final DataBindingContext dbc) {
        ...
        Binding imageBinding = ValueBindingBuilder.bind(imageNameTextObservable).converting(new TrimmingStringConverter())
                .validatingAfterConvert(new DockerImageValidator()).to(imageNameObservable).in(dbc);
    ...
    private void createPushToRegistrySettings(final Composite parent, final DataBindingContext dbc) {
        ...
        ValueBindingBuilder.bind(WidgetProperties.selection().observe(pushImageToRegistryButton)).to(pushImageToRegistryButtonObservable)
                .in(dbc);

        // registry location
        final Label registryLocationLabel = new Label(parent, SWT.NONE);
        registryLocationLabel.setText("Image Registry URL:");
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).indent(30, 0).applyTo(registryLocationLabel);
        final Text registryLocationText = new Text(parent, SWT.BORDER);
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).span(NUM_COLUMS - 1, 1).applyTo(registryLocationText);
        ...
        ValueBindingBuilder.bind(WidgetProperties.text(SWT.Modify).observe(registryLocationText)).to(registryLocationObservable).in(dbc);
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(registryLocationText)).to(pushImageToRegistryButtonObservable).in(dbc);
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).indent(30, 0).applyTo(registryUsernameLabel);
        final Text registryUsernameText = new Text(parent, SWT.BORDER);
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).span(NUM_COLUMS - 1, 1).applyTo(registryUsernameText);
        ...
        ValueBindingBuilder.bind(WidgetProperties.text(SWT.Modify).observe(registryUsernameText)).to(registryUsernameObservable).in(dbc);
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(registryUsernameText)).to(pushImageToRegistryButtonObservable).in(dbc);
        ...
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).indent(30, 0).applyTo(registryPasswordLabel);
        final Text registryPasswordText = new Text(parent, SWT.BORDER + SWT.PASSWORD);
        GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).span(NUM_COLUMS - 1, 1).applyTo(registryPasswordText);
        ...
        ValueBindingBuilder.bind(WidgetProperties.text(SWT.Modify).observe(registryPasswordText)).to(registryPasswordObservable).in(dbc);
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(registryPasswordText)).to(pushImageToRegistryButtonObservable).in(dbc);

        // validation
        final PushImageToRegistryStatusProvider validator = new PushImageToRegistryStatusProvider(pushImageToRegistryButtonObservable,
                registryLocationObservable, registryUsernameObservable, registryPasswordObservable);
        dbc.addValidationStatusProvider(validator);
            ...
            if (pushImageToRegistry) {
                ...
                if (targetRegistryUsername == null || targetRegistryUsername.isEmpty()) {
                    return ValidationStatus
                            .info("The username to authenticate to the target registry is missing. Authentication may fail.");
                }
                if (targetRegistryPassword == null || targetRegistryPassword.isEmpty()) {
                    return ValidationStatus
                            .info("The password to authenticate to the target registry is missing. Authentication may fail.");
                }
            }
            return Status.OK_STATUS;
        }
        ...
        return new IContentProposalProvider() {
            ...
            public IContentProposal[] getProposals(final String input, final int position) {
                return model.getImageNames().stream().filter(name -> name.contains(input))
                        .map(n -> new ContentProposal(n, n, null, position)).toArray(size -> new IContentProposal[size]);
            }
        };
    }
