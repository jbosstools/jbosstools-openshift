...
public class PortForwardingWizardPage extends AbstractOpenShiftWizardPage {
    ...
    protected void doCreateControls(Composite parent, DataBindingContext dbc) {
        ...
        IObservableValue portForwardingAllowedObservable = BeanProperties.value(PortForwardingWizardModel.PROPERTY_PORT_FORWARDING_ALLOWED)
                .observe(wizardModel);

        IObservableValue freePortSearchAllowedObservable = BeanProperties.value(PortForwardingWizardModel.PROPERTY_FREE_PORT_SEARCH_ALLOWED)
                .observe(wizardModel);
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(findFreesPortButton)).notUpdating(freePortSearchAllowedObservable)
                .in(dbc);
                    ...
                    WizardUtils.runInWizard(new Job("Stopping port forwarding...") {
                        ...
                        protected IStatus run(IProgressMonitor monitor) {
                            try {
                                ...
                                Display.getDefault().syncExec(() -> MessageDialog.openError(Display.getDefault().getActiveShell(), "Error",
                                        "Failed to close console inputstream while stopping port-forwarding: " + e.getMessage()));
                                OpenShiftUIActivator.getDefault().getLogger()
                                        .logError("Failed to close console inputstream while stopping port-forwarding", e);
                            }
                            refreshViewerInput(viewer);
            ...
            WizardUtils.runInWizard(new Job("Waiting for ports to get free...") {
                ...
                protected IStatus run(IProgressMonitor monitor) {
                    if (!wizardModel.waitForPortsToGetFree(5)) {
                        Display.getDefault().asyncExec(() -> MessageDialog.openWarning(Display.getDefault().getActiveShell(), "Warning",
                                "Ports remain in use. Try free ports."));
                    }
                    refreshViewerInput(viewer);
                    return Status.OK_STATUS;
                }
