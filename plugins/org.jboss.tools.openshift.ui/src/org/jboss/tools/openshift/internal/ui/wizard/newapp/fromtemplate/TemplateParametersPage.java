...
public class TemplateParametersPage extends AbstractOpenShiftWizardPage {
    ...
    protected void doCreateControls(Composite container, DataBindingContext dbc) {
        ...
        IObservableList<IParameter> parametersObservable = BeanProperties.list(ITemplateParametersPageModel.PROPERTY_PARAMETERS)
                .observe(model);
        ...
        IObservableValue<IParameter> selectedParameter = BeanProperties.value(ITemplateParametersPageModel.PROPERTY_SELECTED_PARAMETER)
                .observe(model);
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(editExistingButton)).notUpdatingParticipant().to(selectedParameter)
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(resetButton)).notUpdatingParticipant().to(selectedParameter)
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(resetAllButton)).notUpdatingParticipant().to(selectedParameter)
                .converting(new IsNotNull2BooleanConverter() {
                    @Override
                    public Object convert(Object fromObject) {
    ...
    public TableViewer createTable(final Composite tableContainer, IObservableList<IParameter> parametersObservable,
        ...
        DataBindingUtils.addDisposableListChangeListener(onParametersChanged(decorations, cellsValidationStatusObservable),
                parametersObservable, table);
        ...
        dbc.addValidationStatusProvider(new MultiValidator() {
            ...
            protected IStatus validate() {
                for (Iterator<IStatus> iterator = (Iterator<IStatus>)cellsValidationStatusObservable.values().iterator(); iterator
                        .hasNext();) {
    ...
    private TableViewer buildTableViewer(Table table, Composite tableContainer,
            ObservableMap<String, IStatus> cellsValidationStatusObservable, TableViewerCellDecorationManager decorations) {
        ...
        TableViewer viewer = builder.contentProvider(new ArrayContentProvider()).column(new CellLabelProvider() {
            ...
            private IStatus validate(IParameter parameter) {
                if (parameter.isRequired()) {
                    if (StringUtils.isEmpty(parameter.getValue()) && StringUtils.isEmpty(parameter.getGeneratorName())) {
    ...
    private void openEditDialog(final IParameter parameter) {
        EditValueDialog dialog = new EditValueDialog(getShell(), "Edit Template Parameter",
                NLS.bind("Please enter a value for {0}.\n{1}", parameter.getName(), parameter.getDescription()), parameter.getName(),
                parameter.getValue(), parameter.isRequired());
