...
public class ExecHelper {
	...
	/**
	 * This method combine <b>out</b> and <b>err</b> outputs in result string, if
	 * you need to have them separately use @link
	 * {@link #executeWithResult(String, boolean, File, Map, String...)}
	 *
	 ...
	 */
	public static ExecResult execute(String executable, boolean checkExitCode, File workingDirectory,
		...
		try {
			try (WriterOutputStream out = new WriterOutputStream(outWriter, Charset.defaultCharset());
					WriterOutputStream err = new WriterOutputStream(errWriter, Charset.defaultCharset())) {
	 ...
	 * @throws IOException if error during process execution
	 */
	public static ExecResult execute(String executable, File workingDirectory, Map<String, String> envs,
	 ...
	 * @throws IOException if error during process execution
	 */
	public static ExecResult execute(String executable, Map<String, String> envs, String... arguments)
			throws IOException {
	...
	private static Process executeWithTerminalInternal(File workingDirectory, Map<String, String> envs,
		...
		Process p = Platform.OS_WIN32.equals(Platform.getOS())
				? ProcessFactory.getFactory().exec(command, env, workingDirectory)
				: ProcessFactory.getFactory().exec(command, env, workingDirectory, new PTY(PTY.Mode.TERMINAL));
		...
		if (listener != null) {
			properties.put(ITerminalsConnectorConstants.PROP_STDERR_LISTENERS,
					new ITerminalServiceOutputStreamMonitorListener[] { listener });
