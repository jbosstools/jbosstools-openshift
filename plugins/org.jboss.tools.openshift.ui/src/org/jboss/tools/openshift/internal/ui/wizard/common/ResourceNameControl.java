...
public class ResourceNameControl {
    ...
    public void doCreateControl(Composite parent, DataBindingContext dbc, Object model) {
        ...
        final Binding nameBinding = ValueBindingBuilder.bind(resourceNameTextObservable).validatingAfterConvert(validator)
                .to(BeanProperties.value(PROPERTY_RESOURCE_NAME).observe(model)).in(dbc);
