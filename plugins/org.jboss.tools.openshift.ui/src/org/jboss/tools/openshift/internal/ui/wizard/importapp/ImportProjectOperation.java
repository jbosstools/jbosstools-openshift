...
public class ImportProjectOperation {
     ...
     */
    public List<IProject> execute(IProgressMonitor monitor) throws OpenShiftException, CoreException, InterruptedException,
    ...
    private List<IProject> importCloning(String gitUrl, String gitRef, File cloneDestination, Collection<String> filters,
    ...
    private List<IProject> importReusingExistingRepo(String gitUrl, String gitRef, File repositoryFolder, Collection<String> filters,
            boolean checkoutBranch, IProgressMonitor monitor) throws CoreException, InterruptedException {
        List<IProject> importedProjects = importProjectsFrom(repositoryFolder, filters, monitor);
        connectToGitRepo(importedProjects, repositoryFolder, monitor);
        if (checkoutBranch) {
        ...
        if (!StringUtils.isEmpty(gitRef) && !CollectionUtils.isEmpty(importedProjects)) {
            ...
            try {
                ...
                throw new CoreException(StatusFactory.errorStatus(OpenShiftCoreActivator.PLUGIN_ID, NLS.bind(
                        "Could check that branch {0} exists within the (reused) local repository at {1}.", gitRef, repositoryFolder), e));
    ...
    protected File cloneRepository(String gitUrl, File destination, String gitRef, IProgressMonitor monitor)
        ...
        EGitUtils.cloneRepository(gitUrl, Constants.DEFAULT_REMOTE_NAME, gitRef, destination, EGitUIUtils.ADD_TO_REPOVIEW_TASK, monitor);
        return destination;
    }
