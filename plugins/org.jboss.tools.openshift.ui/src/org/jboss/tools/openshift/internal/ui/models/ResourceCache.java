...
class ResourceCache {
    ...
    public <T extends IResource> Collection<T> getResources(String namespace, String kind) {
        synchronized (cache) {
            ...
            Map<String, IResource> projectResources = cache.get(namespace);
            if (projectResources != null) {
                return projectResources.values().stream().filter(r -> kind.equals(r.getKind())).map(r -> (T)r).collect(Collectors.toList());
            }
    ...
    public boolean isUpToDate(IResource resource) {
        ...
        return cachedVersion != null
                && Integer.parseInt(cachedVersion.getResourceVersion()) >= Integer.parseInt(resource.getResourceVersion());
    }
}
