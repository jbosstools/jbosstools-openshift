 ...
 */
public class DeleteHandler extends OdoJobHandler {

	@Override
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		ISelection selection = HandlerUtil.getCurrentSelection(event);
		AbstractOpenshiftUIElement<?, ?, ?> element = UIUtils.getFirstElement(selection,
				AbstractOpenshiftUIElement.class);
		if (element == null) {
			return OpenShiftUIActivator.statusFactory().cancelStatus("No element selected"); //$NON-NLS-1$
		}
		String label = getLabel(element);
		if (MessageDialog.openConfirm(HandlerUtil.getActiveShell(event), String.join(" ", "Delete", label),
				String.join(" ", "Are you sure to delete", label, "?"))) {
	...
	private String getLabel(AbstractOpenshiftUIElement<?, ?, ?> element) {
		if (element instanceof NamespaceElement) {
			return String.join(" ", "project", ((NamespaceElement) element).getWrapped());
