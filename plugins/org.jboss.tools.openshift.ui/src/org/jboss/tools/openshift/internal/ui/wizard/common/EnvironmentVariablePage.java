...
public abstract class EnvironmentVariablePage extends AbstractOpenShiftWizardPage {
	...
	protected EnvironmentVariablePage(String title, String description, String name, IWizard wizard,
			IEnvironmentVariablesPageModel model) {
	...
	protected void createEnvVariableControl(Composite parent, DataBindingContext dbc, String label, String tooltip) {
		...
		ValueBindingBuilder.bind(WidgetProperties.enabled().observe(btnReset))
				.notUpdatingParticipant().to(BeanProperties
						.value(IDeploymentConfigPageModel.PROPERTY_SELECTED_ENVIRONMENT_VARIABLE).observe(model))
		...
		ValueBindingBuilder.bind(WidgetProperties.enabled().observe(btnRemove))
				.notUpdatingParticipant().to(BeanProperties
						.value(IDeploymentConfigPageModel.PROPERTY_SELECTED_ENVIRONMENT_VARIABLE).observe(model))
				.converting(new IsNotNull2BooleanConverter() {
	...
	private SelectionListener onResetEnvVar() {
		return new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				EnvironmentVariable envVar = UIUtils.getFirstElement(envViewer.getSelection(),
						EnvironmentVariable.class);
				if (MessageDialog.openQuestion(getShell(), "Reset " + ENVIRONMENT_VARIABLE_LABEL,
						NLS.bind("Are you sure you want to reset the {0} {1}?",
								ENVIRONMENT_VARIABLE_LABEL.toLowerCase(), envVar.getKey()))) {
		...
		return new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (MessageDialog.openQuestion(getShell(), "Reset " + ENVIRONMENT_VARIABLE_LABEL, NLS
						.bind("Are you sure you want to reset all {0}s?", ENVIRONMENT_VARIABLE_LABEL.toLowerCase()))) {
		...
		return new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (MessageDialog.openQuestion(getShell(), "Remove " + ENVIRONMENT_VARIABLE_LABEL,
						NLS.bind("Are you sure you want to remove the {0} {1}?",
								ENVIRONMENT_VARIABLE_LABEL.toLowerCase(), envVar.getKey()))) {
	...
	private class EditHandler extends SelectionAdapter implements IDoubleClickListener {
		...
		private void handleEvent() {
			...
			EnvironmentVariable copy = new EnvironmentVariable(var.getKey(),
					model.isEnvironmentVariableDeleted(var) ? "" : var.getValue(), var.isNew());
