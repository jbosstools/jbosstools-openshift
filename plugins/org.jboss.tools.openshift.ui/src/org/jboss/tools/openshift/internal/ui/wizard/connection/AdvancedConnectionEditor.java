    ...
    public Composite createControls(Composite parent, Object context, DataBindingContext dbc) {
        ...
        DialogAdvancedPart part = new DialogAdvancedPart() {
            ...
            protected void createAdvancedContent(Composite advancedComposite) {
                ...
                GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).span(2, 1).applyTo(txtRegistry);

                registryURLObservable = WidgetProperties.text(SWT.Modify).observeDelayed(DELAY, txtRegistry);
                ValueBindingBuilder.bind(registryURLObservable).validatingAfterConvert(new URLValidator(VALIDATOR_URL_TYPE, true))
                        .converting(new TrimTrailingSlashConverter())
                        .to(BeanProperties.value(AdvancedConnectionEditorModel.PROP_REGISTRY_URL).observe(model)).in(dbc);
                ...
                ValueBindingBuilder.bind(clusterNamespaceObservable).converting(new TrimmingStringConverter())
                        .to(BeanProperties.value(AdvancedConnectionEditorModel.PROP_CLUSTER_NAMESPACE).observe(model)).in(dbc);
                ...
                ValueBindingBuilder.bind(overrideOCObservable)
                        .to(BeanProperties.value(AdvancedConnectionEditorModel.PROP_OC_OVERRIDE).observe(model)).in(dbc);
                ...
                Binding ocLocationBinding = ValueBindingBuilder.bind(ocLocationObservable).converting(new TrimmingStringConverter())
                        .to(BeanProperties.value(AdvancedConnectionEditorModel.PROP_OC_OVERRIDE_LOCATION).observe(model)).in(dbc);
                ...
                ValueBindingBuilder.bind(overrideOCObservable).to(WidgetProperties.enabled().observe(ocText)).notUpdatingParticipant()
                        .in(dbc);
                ...
                ValueBindingBuilder.bind(overrideOCObservable).to(WidgetProperties.enabled().observe(ocBrowse)).notUpdatingParticipant()
                        .in(dbc);
    ...
    private class OCVersionValidationJob extends Job {
        ...
        protected IStatus run(IProgressMonitor monitor) {
            ...
            if (Version.emptyVersion.equals(version)) {
                ...
                this.ocVersionValidity = ValidationStatus
