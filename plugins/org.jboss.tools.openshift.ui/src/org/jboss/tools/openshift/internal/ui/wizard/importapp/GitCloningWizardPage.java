...
public class GitCloningWizardPage extends AbstractOpenShiftWizardPage {
    ...
    public GitCloningWizardPage(IWizard wizard, IGitCloningPageModel model) {
        super(getTitle(model), "Configure the cloning settings by specifying the clone destination", "Cloning settings", wizard);
        this.model = model;
    }

    private static String getTitle(IGitCloningPageModel model) {
        String name = (model == null) ? null : model.getApplicationName();
        return (name == null) ? "Import an existing OpenShift application" : NLS.bind("Import the {0} OpenShift application", name);
    }
    ...
    private Composite createCloneGroup(Composite parent, DataBindingContext dbc) {
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(cloneDestinationText)).notUpdating(useDefaultCloneDestinationObservable)
        ...
        ValueBindingBuilder.bind(WidgetProperties.enabled().observe(reuseRepositoryButton)).notUpdating(repoPathObservable)
                .converting(new FileExistsConverter()).in(dbc);
    ...
    private class CloneDestinationPathValidator extends MultiValidator {
        ...
        protected IStatus validate() {
            ...
            if (!FileUtils.canWrite(cloneDestinationPath.toOSString())) {
                return ValidationStatus.error(NLS.bind("The location {0} is not writeable.", cloneDestinationPath.toOSString()));
            }
        ...
        private IStatus validateReuseGitRepo(final File repoPath, final IPath cloneDestinationPath, boolean cloneDestinationExists) {
            if (!cloneDestinationExists) {
                return ValidationStatus.error(NLS.bind(
            ...
            try {
                ...
                OpenShiftUIActivator.getDefault().getLog()
                        .log(StatusFactory.errorStatus(OpenShiftUIActivator.PLUGIN_ID,
                                NLS.bind("Could not inspect remotes for git repo {0} at {1}.", model.getCloneDestination(),
                                        repoResourcePath.toOSString()),
                                e));
                return false;
            }
        }
