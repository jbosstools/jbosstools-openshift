...
abstract class ResourceContainer<R extends IResource, P extends AbstractOpenshiftUIElement<?, ?>> extends AbstractResourceWrapper<R, P>
    ...
    public Collection<IResourceWrapper<?, ?>> getResourcesOfKind(String kind) {
        return getResources().stream().filter(wrapper -> wrapper.getWrapped().getKind().equals(kind)).collect(Collectors.toSet());
    }
    ...
    protected abstract void postUpdate(Collection<IResource> resources, Map<IResource, AbstractResourceWrapper<?, ?>> updated,
            boolean changed);
