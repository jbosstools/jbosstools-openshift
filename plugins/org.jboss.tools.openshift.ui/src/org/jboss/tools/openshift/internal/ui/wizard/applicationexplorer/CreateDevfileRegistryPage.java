...
public class CreateDevfileRegistryPage extends AbstractOpenShiftWizardPage {
	...
	protected CreateDevfileRegistryPage(IWizard wizard, CreateDevfileRegistryModel model) {
		super("Create devfile registry", "Specify a name and the URL to the registry.",
				"Create devfile registry", wizard);
		this.model = model;
	}
	...
	protected void doCreateControls(Composite parent, DataBindingContext dbc) {
		...
		Binding nameBinding = ValueBindingBuilder.bind(nameObservable)
				...
				.to(BeanProperties.value(CreateDevfileRegistryModel.PROPERTY_NAME).observe(model)).in(dbc);
		...
		Binding urlBinding = ValueBindingBuilder.bind(urlObservable)
				...
				.to(BeanProperties.value(CreateDevfileRegistryModel.PROPERTY_URL).observe(model)).in(dbc);
		ControlDecorationSupport.create(urlBinding, SWT.LEFT | SWT.TOP, null,
				new RequiredControlDecorationUpdater(true));

		Label tokenLabel = new Label(parent, SWT.NONE);
		tokenLabel.setText("Token:");
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).applyTo(tokenLabel);
		Text tokenText = new Text(parent, SWT.BORDER);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).grab(true, false).span(2, 1).applyTo(tokenText);

		ISWTObservableValue<String> tokenObservable = WidgetProperties.text(SWT.Modify).observe(tokenText);
		Binding tokenBinding = ValueBindingBuilder.bind(tokenObservable)
				.to(BeanProperties.value(CreateDevfileRegistryModel.PROPERTY_TOKEN).observe(model)).in(dbc);
