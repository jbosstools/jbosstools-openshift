...
public class DockerHubRegistry {
    ...
    public List<String> getTags(final String repoName) {
        ...
        try {
            ...
            final HttpGet pingApiV1Request = new HttpGet(URIBuilder.target(REGISTRY_LOCATION).path("v1").path("_ping").toString());
            ...
            // repo/name
            final HttpGet getTagsRequest = new HttpGet(
                    URIBuilder.target(REGISTRY_LOCATION).path("v1").path("repositories").path(repoName).path("tags").toString());
